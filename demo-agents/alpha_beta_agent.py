    ...
    if not (termination or truncation):
        ...
        for possible_action in mancala.MancalaModel.ACTIONS(env.my_state):
            ...
            if env1.my_state.turn == MY_SIDE:
                val = MAX_AB(env1, 1, alpha, beta)
            else:
                val = MIN_AB(env1, 2, alpha, beta)
...
def MAX_AB(env, depth, alpha, beta):
    ...
    for possible_action in mancala.MancalaModel.ACTIONS(env.my_state):
        ...
        env1.step(possible_action)
        if env1.my_state.turn == MY_SIDE:
            v = MAX_AB(env1, depth, alpha, beta)
        else:
            v = MIN_AB(env1, depth+1, alpha, beta)            
        if v > v_max:
            v_max = v
            if v == float("inf") or v >= beta:
                break
            if v > alpha:
                alpha = v
    return v_max
...
def MIN_AB(env, depth, alpha, beta):
    winning_score = IS_WINNING_HEURISTIC(env)
    if depth >= MAX_DEPTH:
        return EVALUATE(env)
    elif winning_score is not None:
    ...
    for possible_action in mancala.MancalaModel.ACTIONS(env.my_state):
        ...
        env1.step(possible_action)
        if env1.my_state.turn == MY_SIDE:
            v = MAX_AB(env1, depth+1, alpha, beta)
        else:
            v = MIN_AB(env1, depth, alpha, beta)
        if v < v_min:
            v_min = v
            if v == float("-inf") or v <= alpha:
                break
            if v < beta:
                beta = v
    return v_min
